...
bash "$ROOT/tests/docs_index.sh" "$ROOT" >/tmp/l0_docs_index.out
if ! grep -q '^ok$' /tmp/l0_docs_index.out; then
  echo "FAIL: docs index gate did not report ok"
  exit 1
fi

bash "$ROOT/tests/docs_snapshot.sh" "$ROOT" v1.0.0 >/tmp/l0_docs_snapshot.out
if ! grep -q '^ok$' /tmp/l0_docs_snapshot.out; then
  echo "FAIL: docs snapshot gate did not report ok"
  exit 1
fi
...
bash "$ROOT/tests/verifier_matrix.sh" "$BIN" "$ROOT/tests/verifier_matrix.tsv" >/tmp/l0_m53_verifier_matrix.out
if ! grep -q '^ok$' /tmp/l0_m53_verifier_matrix.out; then
  echo "FAIL: verifier matrix did not report ok"
  exit 1
fi
...
bash "$ROOT/tests/debug_map_schema.sh" "$BIN" >/tmp/l0_m61_debug_map_schema.out
if ! grep -q '^ok$' /tmp/l0_m61_debug_map_schema.out; then
  echo "FAIL: debug map schema contracts did not report ok"
  exit 1
fi
...
bash "$ROOT/tests/m65_fuzz_stress.sh" "$BIN" "$ROOT" >/tmp/l0_m65_fuzz_stress.out
if ! grep -q '^ok$' /tmp/l0_m65_fuzz_stress.out; then
  echo "FAIL: M65 fuzz stress did not report ok"
  exit 1
fi
...
bash "$ROOT/tests/error_model.sh" "$BIN" "$ROOT" >/tmp/l0_m67_error_model.out
if ! grep -q '^ok$' /tmp/l0_m67_error_model.out; then
  echo "FAIL: M67 error model did not report ok"
  exit 1
fi

bash "$ROOT/tests/release_pipeline.sh" "$BIN" "$ROOT" >/tmp/l0_m68_release_pipeline.out
if ! grep -q '^ok$' /tmp/l0_m68_release_pipeline.out; then
  echo "FAIL: M68 release pipeline did not report ok"
  exit 1
fi
...
if ! grep -q '^ok$' /tmp/l0_ok_merge_mem_select_lowered.out; then
  echo "FAIL: verify valid_merge_mem_select_lowered"
  exit 1
fi
"$BIN" verify "$ROOT/tests/valid_merge_mem_select_with_dead_const_general_lowered.l0" >/tmp/l0_ok_merge_mem_select_with_dead_const_general_lowered.out
if ! grep -q '^ok$' /tmp/l0_ok_merge_mem_select_with_dead_const_general_lowered.out; then
  echo "FAIL: verify valid_merge_mem_select_with_dead_const_general_lowered"
  exit 1
fi
"$BIN" verify "$ROOT/tests/valid_merge_mem_select_guardrail_fallback.l0" >/tmp/l0_ok_merge_mem_select_guardrail_fallback.out
if ! grep -q '^ok$' /tmp/l0_ok_merge_mem_select_guardrail_fallback.out; then
...
"$BIN" verify "$ROOT/tests/valid_call_and_swapped_lowered.l0" >/tmp/l0_ok_call_and_swapped_lowered.out
if ! grep -q '^ok$' /tmp/l0_ok_call_and_swapped_lowered.out; then
  echo "FAIL: verify valid_call_and_swapped_lowered"
  exit 1
fi
"$BIN" verify "$ROOT/tests/valid_call_and_with_dead_const_general_lowered.l0" >/tmp/l0_ok_call_and_with_dead_const_general_lowered.out
if ! grep -q '^ok$' /tmp/l0_ok_call_and_with_dead_const_general_lowered.out; then
  echo "FAIL: verify valid_call_and_with_dead_const_general_lowered"
  exit 1
fi
"$BIN" verify "$ROOT/tests/valid_call_or_lowered.l0" >/tmp/l0_ok_call_or_lowered.out
if ! grep -q '^ok$' /tmp/l0_ok_call_or_lowered.out; then
  echo "FAIL: verify valid_call_or_lowered"
  exit 1
fi
"$BIN" verify "$ROOT/tests/valid_call_or_swapped_lowered.l0" >/tmp/l0_ok_call_or_swapped_lowered.out
if ! grep -q '^ok$' /tmp/l0_ok_call_or_swapped_lowered.out; then
  echo "FAIL: verify valid_call_or_swapped_lowered"
  exit 1
fi
"$BIN" verify "$ROOT/tests/valid_call_or_with_dead_const_general_lowered.l0" >/tmp/l0_ok_call_or_with_dead_const_general_lowered.out
if ! grep -q '^ok$' /tmp/l0_ok_call_or_with_dead_const_general_lowered.out; then
...
"$BIN" verify "$ROOT/tests/valid_malloc_with_dead_icmp_general_lowered.l0" >/tmp/l0_ok_malloc_with_dead_icmp_general_lowered.out
if ! grep -q '^ok$' /tmp/l0_ok_malloc_with_dead_icmp_general_lowered.out; then
  echo "FAIL: verify valid_malloc_with_dead_icmp_general_lowered"
  exit 1
fi
"$BIN" verify "$ROOT/tests/valid_free_noop_with_dead_icmp_general_lowered.l0" >/tmp/l0_ok_free_noop_with_dead_icmp_general_lowered.out
if ! grep -q '^ok$' /tmp/l0_ok_free_noop_with_dead_icmp_general_lowered.out; then
  echo "FAIL: verify valid_free_noop_with_dead_icmp_general_lowered"
  exit 1
fi
"$BIN" verify "$ROOT/tests/valid_write_newline_with_dead_icmp_general_lowered.l0" >/tmp/l0_ok_write_newline_with_dead_icmp_general_lowered.out
if ! grep -q '^ok$' /tmp/l0_ok_write_newline_with_dead_icmp_general_lowered.out; then
  echo "FAIL: verify valid_write_newline_with_dead_icmp_general_lowered"
  exit 1
fi
"$BIN" verify "$ROOT/tests/valid_trace_noop_with_dead_icmp_general_lowered.l0" >/tmp/l0_ok_trace_noop_with_dead_icmp_general_lowered.out
if ! grep -q '^ok$' /tmp/l0_ok_trace_noop_with_dead_icmp_general_lowered.out; then
  echo "FAIL: verify valid_trace_noop_with_dead_icmp_general_lowered"
  exit 1
fi
"$BIN" verify "$ROOT/tests/valid_exit_with_dead_icmp_general_lowered.l0" >/tmp/l0_ok_exit_with_dead_icmp_general_lowered.out
if ! grep -q '^ok$' /tmp/l0_ok_exit_with_dead_icmp_general_lowered.out; then
  echo "FAIL: verify valid_exit_with_dead_icmp_general_lowered"
  exit 1
fi
"$BIN" verify "$ROOT/tests/valid_write_newline_alloca0_with_dead_icmp_guardrail_fallback.l0" >/tmp/l0_ok_write_newline_alloca0_with_dead_icmp_guardrail_fallback.out
if ! grep -q '^ok$' /tmp/l0_ok_write_newline_alloca0_with_dead_icmp_guardrail_fallback.out; then
...
call_and_dbg_off=$(od -An -t u8 -j 64 -N 8 /tmp/l0_test_call_and_lowered.img | tr -d ' ')
call_and_kernel_kind=$(od -An -t u8 -j "$((call_and_dbg_off + 32))" -N 8 /tmp/l0_test_call_and_lowered.img | tr -d ' ')
if [ "$call_and_kernel_kind" != "5" ]; then
  echo "FAIL: call->and debug kernel kind id"
  exit 1
fi
"$BIN" run /tmp/l0_test_call_and_lowered.img 42 15 >/tmp/l0_run_call_and_lowered.out
if [ "$(tr -d '\n' < /tmp/l0_run_call_and_lowered.out)" != "10" ]; then
  echo "FAIL: run call->and lowered image result"
  exit 1
fi
"$BIN" build "$ROOT/tests/valid_call_and_swapped_lowered.l0" /tmp/l0_test_call_and_swapped_lowered.img >/tmp/l0_build_call_and_swapped_lowered.out
if ! grep -q '^ok$' /tmp/l0_build_call_and_swapped_lowered.out; then
  echo "FAIL: build valid_call_and_swapped_lowered"
  exit 1
fi
call_and_swapped_dbg_off=$(od -An -t u8 -j 64 -N 8 /tmp/l0_test_call_and_swapped_lowered.img | tr -d ' ')
call_and_swapped_kernel_kind=$(od -An -t u8 -j "$((call_and_swapped_dbg_off + 32))" -N 8 /tmp/l0_test_call_and_swapped_lowered.img | tr -d ' ')
if [ "$call_and_swapped_kernel_kind" != "5" ]; then
  echo "FAIL: call->and swapped debug kernel kind id"
  exit 1
fi
"$BIN" run /tmp/l0_test_call_and_swapped_lowered.img 42 15 >/tmp/l0_run_call_and_swapped_lowered.out
if [ "$(tr -d '\n' < /tmp/l0_run_call_and_swapped_lowered.out)" != "10" ]; then
  echo "FAIL: run call->and swapped lowered image result"
  exit 1
fi
"$BIN" build "$ROOT/tests/valid_call_and_with_dead_const_general_lowered.l0" /tmp/l0_test_call_and_with_dead_const_general_lowered.img >/tmp/l0_build_call_and_with_dead_const_general_lowered.out
if ! grep -q '^ok$' /tmp/l0_build_call_and_with_dead_const_general_lowered.out; then
  echo "FAIL: build valid_call_and_with_dead_const_general_lowered"
  exit 1
fi
call_and_dead_const_dbg_off=$(od -An -t u8 -j 64 -N 8 /tmp/l0_test_call_and_with_dead_const_general_lowered.img | tr -d ' ')
call_and_dead_const_kernel_kind=$(od -An -t u8 -j "$((call_and_dead_const_dbg_off + 32))" -N 8 /tmp/l0_test_call_and_with_dead_const_general_lowered.img | tr -d ' ')
if [ "$call_and_dead_const_kernel_kind" != "5" ]; then
  echo "FAIL: call->and with dead const debug kernel kind id"
  exit 1
fi
"$BIN" run /tmp/l0_test_call_and_with_dead_const_general_lowered.img 42 15 >/tmp/l0_run_call_and_with_dead_const_general_lowered.out
if [ "$(tr -d '\n' < /tmp/l0_run_call_and_with_dead_const_general_lowered.out)" != "10" ]; then
  echo "FAIL: run call->and with dead const lowered image result"
  exit 1
fi
"$BIN" build "$ROOT/tests/valid_call_or_lowered.l0" /tmp/l0_test_call_or_lowered.img >/tmp/l0_build_call_or_lowered.out
if ! grep -q '^ok$' /tmp/l0_build_call_or_lowered.out; then
  echo "FAIL: build valid_call_or_lowered"
  exit 1
fi
call_or_dbg_off=$(od -An -t u8 -j 64 -N 8 /tmp/l0_test_call_or_lowered.img | tr -d ' ')
call_or_kernel_kind=$(od -An -t u8 -j "$((call_or_dbg_off + 32))" -N 8 /tmp/l0_test_call_or_lowered.img | tr -d ' ')
if [ "$call_or_kernel_kind" != "6" ]; then
  echo "FAIL: call->or debug kernel kind id"
  exit 1
fi
"$BIN" run /tmp/l0_test_call_or_lowered.img 42 15 >/tmp/l0_run_call_or_lowered.out
if [ "$(tr -d '\n' < /tmp/l0_run_call_or_lowered.out)" != "47" ]; then
  echo "FAIL: run call->or lowered image result"
  exit 1
fi
"$BIN" build "$ROOT/tests/valid_call_or_swapped_lowered.l0" /tmp/l0_test_call_or_swapped_lowered.img >/tmp/l0_build_call_or_swapped_lowered.out
if ! grep -q '^ok$' /tmp/l0_build_call_or_swapped_lowered.out; then
  echo "FAIL: build valid_call_or_swapped_lowered"
  exit 1
fi
call_or_swapped_dbg_off=$(od -An -t u8 -j 64 -N 8 /tmp/l0_test_call_or_swapped_lowered.img | tr -d ' ')
call_or_swapped_kernel_kind=$(od -An -t u8 -j "$((call_or_swapped_dbg_off + 32))" -N 8 /tmp/l0_test_call_or_swapped_lowered.img | tr -d ' ')
if [ "$call_or_swapped_kernel_kind" != "6" ]; then
  echo "FAIL: call->or swapped debug kernel kind id"
  exit 1
fi
"$BIN" run /tmp/l0_test_call_or_swapped_lowered.img 42 15 >/tmp/l0_run_call_or_swapped_lowered.out
if [ "$(tr -d '\n' < /tmp/l0_run_call_or_swapped_lowered.out)" != "47" ]; then
  echo "FAIL: run call->or swapped lowered image result"
  exit 1
fi
"$BIN" build "$ROOT/tests/valid_call_or_with_dead_const_general_lowered.l0" /tmp/l0_test_call_or_with_dead_const_general_lowered.img >/tmp/l0_build_call_or_with_dead_const_general_lowered.out
if ! grep -q '^ok$' /tmp/l0_build_call_or_with_dead_const_general_lowered.out; then
  echo "FAIL: build valid_call_or_with_dead_const_general_lowered"
  exit 1
fi
call_or_dead_const_dbg_off=$(od -An -t u8 -j 64 -N 8 /tmp/l0_test_call_or_with_dead_const_general_lowered.img | tr -d ' ')
call_or_dead_const_kernel_kind=$(od -An -t u8 -j "$((call_or_dead_const_dbg_off + 32))" -N 8 /tmp/l0_test_call_or_with_dead_const_general_lowered.img | tr -d ' ')
if [ "$call_or_dead_const_kernel_kind" != "6" ]; then
  echo "FAIL: call->or with dead const debug kernel kind id"
  exit 1
fi
...
"$BIN" build "$ROOT/tests/valid_merge_mem_select_lowered.l0" /tmp/l0_test_merge_mem_select.img >/tmp/l0_build_merge_mem_select.out
if ! grep -q '^ok$' /tmp/l0_build_merge_mem_select.out; then
  echo "FAIL: build valid_merge_mem_select_lowered"
  exit 1
fi
merge_mem_select_dbg_off=$(od -An -t u8 -j 64 -N 8 /tmp/l0_test_merge_mem_select.img | tr -d ' ')
merge_mem_select_kernel_kind=$(od -An -t u8 -j "$((merge_mem_select_dbg_off + 32))" -N 8 /tmp/l0_test_merge_mem_select.img | tr -d ' ')
if [ "$merge_mem_select_kernel_kind" != "27" ]; then
  echo "FAIL: merge mem-select kernel kind"
  exit 1
fi
"$BIN" run /tmp/l0_test_merge_mem_select.img 1 >/tmp/l0_run_merge_mem_select_true.out
if [ "$(tr -d '\n' < /tmp/l0_run_merge_mem_select_true.out)" != "11" ]; then
  echo "FAIL: run merge mem-select true result"
  exit 1
fi
"$BIN" run /tmp/l0_test_merge_mem_select.img 0 >/tmp/l0_run_merge_mem_select_false.out
if [ "$(tr -d '\n' < /tmp/l0_run_merge_mem_select_false.out)" != "22" ]; then
  echo "FAIL: run merge mem-select false result"
  exit 1
fi
"$BIN" build "$ROOT/tests/valid_merge_mem_select_lowered.l0" /tmp/l0_test_merge_mem_select_map.img --debug-map /tmp/l0_merge_mem_select_debug_map.bin >/tmp/l0_build_merge_mem_select_map.out
if ! grep -q '^ok$' /tmp/l0_build_merge_mem_select_map.out; then
  echo "FAIL: build valid_merge_mem_select_lowered with --debug-map"
  exit 1
fi
"$BIN" mapcat /tmp/l0_merge_mem_select_debug_map.bin >/tmp/l0_merge_mem_select_mapcat.out
if [ "$(cat /tmp/l0_merge_mem_select_mapcat.out)" != $'entries 3\ncode_size 27\ninst_id 1\nstart 0\nend 5\ninst_id 2\nstart 5\nend 16\ninst_id 3\nstart 16\nend 27' ]; then
  echo "FAIL: merge mem-select debug-map layout"
  exit 1
fi
"$BIN" build "$ROOT/tests/valid_merge_mem_select_with_dead_const_general_lowered.l0" /tmp/l0_test_merge_mem_select_dead_const.img >/tmp/l0_build_merge_mem_select_dead_const.out
if ! grep -q '^ok$' /tmp/l0_build_merge_mem_select_dead_const.out; then
  echo "FAIL: build valid_merge_mem_select_with_dead_const_general_lowered"
  exit 1
fi
merge_mem_select_dead_dbg_off=$(od -An -t u8 -j 64 -N 8 /tmp/l0_test_merge_mem_select_dead_const.img | tr -d ' ')
merge_mem_select_dead_kernel_kind=$(od -An -t u8 -j "$((merge_mem_select_dead_dbg_off + 32))" -N 8 /tmp/l0_test_merge_mem_select_dead_const.img | tr -d ' ')
if [ "$merge_mem_select_dead_kernel_kind" != "27" ]; then
  echo "FAIL: merge mem-select dead-const kernel kind"
  exit 1
fi
"$BIN" build "$ROOT/tests/valid_merge_mem_select_guardrail_fallback.l0" /tmp/l0_test_merge_mem_select_guardrail_fallback.img >/tmp/l0_build_merge_mem_select_guardrail_fallback.out
if ! grep -q '^ok$' /tmp/l0_build_merge_mem_select_guardrail_fallback.out; then
  echo "FAIL: build valid_merge_mem_select_guardrail_fallback"
  exit 1
fi
merge_mem_select_guardrail_dbg_off=$(od -An -t u8 -j 64 -N 8 /tmp/l0_test_merge_mem_select_guardrail_fallback.img | tr -d ' ')
...
"$BIN" build "$ROOT/tests/valid_malloc_with_dead_icmp_general_lowered.l0" /tmp/l0_test_malloc_with_dead_icmp_general_lowered.img >/tmp/l0_build_malloc_with_dead_icmp_general_lowered.out
if ! grep -q '^ok$' /tmp/l0_build_malloc_with_dead_icmp_general_lowered.out; then
  echo "FAIL: build valid_malloc_with_dead_icmp_general_lowered"
  exit 1
fi
if [ "$(get_kernel_kind /tmp/l0_test_malloc_with_dead_icmp_general_lowered.img)" != "20" ]; then
  echo "FAIL: malloc dead-icmp generalized kernel kind"
  exit 1
fi
"$BIN" run /tmp/l0_test_malloc_with_dead_icmp_general_lowered.img 4096 >/tmp/l0_run_malloc_with_dead_icmp_general_lowered.out
malloc_dead_icmp_out="$(tr -d '\n' < /tmp/l0_run_malloc_with_dead_icmp_general_lowered.out)"
if [ "$malloc_dead_icmp_out" = "0" ]; then
...
"$BIN" build "$ROOT/tests/valid_exit_with_dead_icmp_general_lowered.l0" /tmp/l0_test_exit_with_dead_icmp_general_lowered.img >/tmp/l0_build_exit_with_dead_icmp_general_lowered.out
if ! grep -q '^ok$' /tmp/l0_build_exit_with_dead_icmp_general_lowered.out; then
  echo "FAIL: build valid_exit_with_dead_icmp_general_lowered"
  exit 1
fi
if [ "$(get_kernel_kind /tmp/l0_test_exit_with_dead_icmp_general_lowered.img)" != "23" ]; then
  echo "FAIL: exit dead-icmp generalized kernel kind"
  exit 1
fi
set +e
"$BIN" run /tmp/l0_test_exit_with_dead_icmp_general_lowered.img 19 >/tmp/l0_run_exit_with_dead_icmp_general_lowered.out 2>/tmp/l0_run_exit_with_dead_icmp_general_lowered.err
exit_dead_icmp_rc=$?
set -e
...
"$BIN" build "$ROOT/tests/valid_write_newline_with_dead_icmp_general_lowered.l0" /tmp/l0_test_write_newline_with_dead_icmp_general_lowered.img >/tmp/l0_build_write_newline_with_dead_icmp_general_lowered.out
if ! grep -q '^ok$' /tmp/l0_build_write_newline_with_dead_icmp_general_lowered.out; then
  echo "FAIL: build valid_write_newline_with_dead_icmp_general_lowered"
  exit 1
fi
if [ "$(get_kernel_kind /tmp/l0_test_write_newline_with_dead_icmp_general_lowered.img)" != "22" ]; then
  echo "FAIL: write newline dead-icmp generalized lowering kernel kind"
  exit 1
fi
"$BIN" run /tmp/l0_test_write_newline_with_dead_icmp_general_lowered.img >/tmp/l0_run_write_newline_with_dead_icmp_general_lowered.out
if [ "$(tr -d '\n' < /tmp/l0_run_write_newline_with_dead_icmp_general_lowered.out)" != "0" ]; then
  echo "FAIL: run write newline dead-icmp generalized image result"
  exit 1
fi
if [ "$(od -An -t x1 /tmp/l0_run_write_newline_with_dead_icmp_general_lowered.out | tr -d ' \n')" != "0a300a" ]; then
...
if ! grep -q '^ok$' /tmp/l0_build_free_noop_with_dead_icmp_general_lowered.out; then
  echo "FAIL: build valid_free_noop_with_dead_icmp_general_lowered"
  exit 1
fi
if [ "$(get_kernel_kind /tmp/l0_test_free_noop_with_dead_icmp_general_lowered.img)" != "21" ]; then
  echo "FAIL: free noop dead-icmp generalized lowering kernel kind"
  exit 1
fi
...
"$BIN" build "$ROOT/tests/valid_trace_noop_with_dead_icmp_general_lowered.l0" /tmp/l0_test_trace_noop_with_dead_icmp_general_lowered.img >/tmp/l0_build_trace_noop_with_dead_icmp_general_lowered.out
if ! grep -q '^ok$' /tmp/l0_build_trace_noop_with_dead_icmp_general_lowered.out; then
  echo "FAIL: build valid_trace_noop_with_dead_icmp_general_lowered"
  exit 1
fi
if [ "$(get_kernel_kind /tmp/l0_test_trace_noop_with_dead_icmp_general_lowered.img)" != "24" ]; then
  echo "FAIL: trace noop dead-icmp generalized lowering kernel kind"
  exit 1
fi
"$BIN" run /tmp/l0_test_trace_noop_with_dead_icmp_general_lowered.img 123 >/tmp/l0_run_trace_noop_with_dead_icmp_general_lowered.out 2>/tmp/l0_run_trace_noop_with_dead_icmp_general_lowered.err
if [ "$(tr -d '\n' < /tmp/l0_run_trace_noop_with_dead_icmp_general_lowered.out)" != "0" ]; then
  echo "FAIL: run trace noop dead-icmp generalized image result"
  exit 1
fi
...
"$BIN" build "$ROOT/tests/valid_write_newline_alloca0_with_dead_icmp_guardrail_fallback.l0" /tmp/l0_test_write_newline_alloca0_with_dead_icmp_guardrail_fallback.img >/tmp/l0_build_write_newline_alloca0_with_dead_icmp_guardrail_fallback.out
if ! grep -q '^ok$' /tmp/l0_build_write_newline_alloca0_with_dead_icmp_guardrail_fallback.out; then
  echo "FAIL: build valid_write_newline_alloca0_with_dead_icmp_guardrail_fallback"
  exit 1
fi
