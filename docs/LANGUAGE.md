...
- I currently emit one of two bootstrap code payloads:
  - canonical lowered kernel payloads for:
    ...
    - for const-dependent kernels (`free`, `write`, `trace`), I currently keep exact canonical template matching as the lowering authority; generalized normalization hooks are active in the build chain with deterministic legacy fallback behavior, and I track dead-const lowering closure for these kernels in M20
