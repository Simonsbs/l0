...
- I currently emit one of two bootstrap code payloads:
  - canonical lowered kernel payloads for:
    ...
    - for const-dependent kernels (`free`, `write`, `trace`), I currently keep exact canonical template matching as the lowering authority; I have staged generalized normalization hooks in the build chain and track dead-const lowering closure for these kernels in M19
