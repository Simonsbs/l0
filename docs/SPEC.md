...
- I then write bootstrap code section bytes at `code_off`:
  ...
  - I lower canonical intrinsic kernel shapes:
    ...
    - for const-dependent intrinsic shapes (`free`, `write`, `trace`), I still require exact canonical templates as lowering authority today; generalized normalization hooks are active in the build chain with deterministic legacy fallback behavior, and I track dead-const lowering closure for these in M20
